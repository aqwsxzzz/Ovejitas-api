...
async def _continuous_expected(
    db: AsyncSession, window: Window, targets: list[AssetProductionTarget]
) -> Decimal:
...
async def _expected(
    ...
    return await _continuous_expected(db, window, targets)
...
async def production_productivity(
    ...
    for asset_id, category_id in pairs:
        ...
        if target_list:
            expected = await _expected(db, window, target_list, prod["count"])
            if expected > 0:
