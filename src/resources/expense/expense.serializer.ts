...
export class ExpenseSerializer {
	static serialize(expense: ExpenseModel): ExpenseResponse {
		return {
			...
			paymentMethod: expense.paymentMethod as ExpenseResponse['paymentMethod'] ?? undefined,
			invoiceNumber: expense.invoiceNumber ?? '',
			quantity: expense.quantity ?? 0,
			quantityUnit: expense.quantityUnit as ExpenseResponse['quantityUnit'] ?? undefined,
			unitCost: expense.unitCost ?? 0,
			status: expense.status as ExpenseResponse['status'] ?? undefined,
